...
exports.getAllAdHocJobs = function(req, res) {
    ...
    var callback = function(err, adHocJobs) {
        if (err) {
            ...
            detailLogger.debug(' GET - Retrieved all AdHoc Jobs by ', {
                clientIPaddress: clientIPaddress
            });
            res.send(adHocJobs);
        }
...
exports.submitNewAdHocJob = function(req, res) {
    ...
    if (!jobID) {
        ...
        detailLogger.info(' JobID - %s New AdHoc Job Submitted: %s', jobID, JSON.stringify({
            clientIPaddress: clientIPaddress,
            JobName: jobName,
            sqlQuery: sqlQuery
        }));
        highLevelLogger.info(' JobID - %s New AdHoc Job Submitted: %s', jobID, JSON.stringify({
            clientIPaddress: clientIPaddress,
            JobName: jobName,
            sqlQuery: sqlQuery
        }));
        ...
        function callback(err) {
            if (err) {
                detailLogger.error('JobID - %s Error updating the JobRunStatus to %s in database : %s', jobID, jobStatus, JSON.stringify({
                    error: err
                }));
            } else {
        ...
        fs.ensureDir(jobDataDir, function(err) {
            if (err) {
                ...
                fs.outputFile(queryFile, sqlQuery, function(err) {
                    if (err) {
                        detailLogger.error('JobID - %s Error creating Query file: %s', jobID, JSON.stringify({
                            QueryFile: queryFile,
                            error: err
                        }));
                        res.status(500);
    ...
    executeHiveScript = function() {
        ...
        function callback(error, results) {
            if (error) {
                detailLogger.error('JobID - %s  Execution of Hive Query failed: %s', jobID, JSON.stringify({
                    error: error
                }));
                highLevelLogger.error('JobID - %s  Execution of Hive Query failed: %s', jobID, JSON.stringify({
                    error: error
                }));
                highLevelLogger.info(' JobID - %s  AdHoc Job failed: %s', jobID, JSON.stringify({
                    clientIPaddress: clientIPaddress,
                    JobName: jobName,
                    sqlQuery: sqlQuery,
                    error: error
        ...
        detailLogger.debug('JobID - %s  Executing HiveScriptLauncher: %s', jobID, JSON.stringify({
            scriptName: execFileName,
            args: args
        }));
    ...
    updateStatusInDB = function(status) {
        ...
        function callback(err) {
            if (err) {
                detailLogger.error('JobID - %s Error updating the AdHoc Job status to %s in Database: %s', jobID, status, JSON.stringify({
                    error: err
                }));
            } else {
...
exports.getAdHocJobStatus = function(req, res) {
    ...
    if (jobID) {
        ...
        var callback = function(err, adHocJob) {
            if (err) {
                detailLogger.error('JobID - %s Error fetching Job Status %s', jobID, JSON.stringify({
                    clientIPaddress: clientIPaddress,
                    error: err
                }));
                highLevelLogger.error('JobID - %s Error fetching Job Status %s', jobID, JSON.stringify({
                    clientIPaddress: clientIPaddress,
                    error: err
                }));
                res.status(500)
...
exports.updateStatus = function(req, res) {
    ...
    return AdHocJob.findById(JobID, function(err, adHocJob) {
        ...
        return adHocJob.save(function(err) {
            if (err) {
                highLevelLogger.error('JobID - %s Error updating Job Status %s', jobID, JSON.stringify({
                    clientIPaddress: clientIPaddress,
                    error: err
                }));
                detailLogger.error('JobID - %s Error updating Job Status %s', jobID, JSON.stringify({
                    clientIPaddress: clientIPaddress,
                    error: err
                }));
                highLevelLogger.error('JobID - %s Error updating Job Status %s', jobID, JSON.stringify({
